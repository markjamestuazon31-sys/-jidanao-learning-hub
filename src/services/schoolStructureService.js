...
export async function getSchoolStructure() {
  ...
  return snapshot.exists()
    ? normalizeSchoolStructure(snapshot.val(), { fallback: false })
...
export async function getManagedSchoolStructure() {
  ...
  return snapshot.exists()
    ? normalizeSchoolStructure(snapshot.val())
    : normalizeSchoolStructure(DEFAULT_SCHOOL_STRUCTURE);
}
...
export async function saveSchoolStructure(structure, administratorUid = "") {
  const normalized = normalizeSchoolStructure(structure, { fallback: false });
